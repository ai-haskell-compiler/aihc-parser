...
_Published on February 23, 2019, last updated October 30, 2021_

_This is the Megaparsec tutorial which originally was written as a chapter_
_for the [Intermediate Haskell](https://intermediatehaskell.com/) book. Due to lack of progress with the_
_book in the last year, other authors agreed to let me publish the text as a_
_standalone tutorial so that people can benefit at least from this part of_
_our work._

[Japanese translation](https://haskell.e-bigmoon.com/posts/2019/07-14-megaparsec-tutorial.html), [Chinese translation](https://blog.yzyzsun.me/megaparsec/).

- [`ParsecT` and `Parsec` monads](#parsect-and-parsec-monads)
- [Character and binary streams](#character-and-binary-streams)
- [Monadic and applicative syntax](#monadic-and-applicative-syntax)
- [Forcing consumption of input with `eof`](#forcing-consumption-of-input-with-eof)
- [Working with alternatives](#working-with-alternatives)
- [Controlling backtracking with `try`](#controlling-backtracking-with-try)
- [Debugging parsers](#debugging-parsers)
- [Labeling and hiding things](#labeling-and-hiding-things)
- [Running a parser](#running-a-parser)
- [The `MonadParsec` type class](#the-monadparsec-type-class)
- [Lexing](#lexing) - [White space](#white-space)
  - [Char and string literals](#char-and-string-literals)
  - [Numbers](#numbers)
- [`notFollowedBy` and `lookAhead`](#notfollowedby-and-lookahead)
- [Parsing expressions](#parsing-expressions)
- [Indentation-sensitive parsing](#indentationsensitive-parsing) - [`nonIndented` and `indentBlock`](#nonindented-and-indentblock)
  - [Parsing a simple indented list](#parsing-a-simple-indented-list)
  - [Nested indented list](#nested-indented-list)
  - [Adding line folds](#adding-line-folds)
- [Writing efficient parsers](#writing-efficient-parsers)
- [Parse errors](#parse-errors) - [Parse error definitions](#parse-error-definitions)
  - [How to signal a parse error](#how-to-signal-a-parse-error)
  - [Displaying parse errors](#displaying-parse-errors)
  - [Catching parse errors in a running parser](#catching-parse-errors-in-a-running-parser)
  - [Controlling location of parse errors](#controlling-location-of-parse-errors)
  - [Reporting multiple parse errors](#reporting-multiple-parse-errors)
- [Testing Megaparsec parsers](#testing-megaparsec-parsers)
- [Working with custom input streams](#working-with-custom-input-streams)
...
- [`parsec`](https://hackage.haskell.org/package/parsec) has been the “default” parsing library in Haskell for a
...
- [`attoparsec`](https://hackage.haskell.org/package/attoparsec) is a robust, fast parsing library with focus on
...
- [`trifecta`](https://hackage.haskell.org/package/trifecta) features good error messages but is
  under-documented and hard to figure out. It can parse `String` and
  `ByteString` out-of-the-box, but not `Text`.

- [`megaparsec`](https://hackage.haskell.org/package/megaparsec) is a fork of `parsec` that has been actively
...
## `ParsecT` and `Parsec` monads [Anchor](#parsect-and-parsec-monads)
...
such as `MonadState` and `MonadReader`. We will return to `MonadParsec` [later](#the-monadparsec-type-class) and discuss it in more details.
...
- Working with concrete types with all type variables fixed helps GHC
  ...
  into so-called _interface files_ will allow GHC produce very efficient
...
## Character and binary streams [Anchor](#character-and-binary-streams)
...
- `Tokens s` for stream `s` is the type of a “chunk” of stream. The concept
  of _chunk_ was only introduced for performance reasons. Indeed, it is
...
_The `:: Parser Char` annotation is necessary because `satisfy` by itself is_
_polymorphic, so `parseTest` cannot know what to use in place of `e` and `s`_
_in `MonadParsec e s m` (`m` is assumed to be `Identity` with these helpers)._
_If we worked with a pre-existing parser which had a type signature, the_
_explicit clarification of parser type would be unnecessary._
...
## Monadic and applicative syntax [Anchor](#monadic-and-applicative-syntax)
...
## Forcing consumption of input with `eof` [Anchor](#forcing-consumption-of-input-with-eof)
...
type class. The [`parser-combinators`](https://hackage.haskell.org/package/parser-combinators) package provides
...
given parser _zero_ or more times:
...
of input is nothing but a concept, there is a primitive called `eof :: MonadParsec e s m => m ()` that does not ever consume anything and only
...
## Working with alternatives [Anchor](#working-with-alternatives)
...
_`choice` is just a synonym for `asum`—an operation that folds a list_
_putting `(<|>)` between its elements, so the two definitions of `pScheme`_
_are actually the same, although the one which uses `choice` may look a bit_
_nicer._
...
_The `(<$)` operator just puts the value on its left-hand side into a_
_functorial context replacing whatever is there at the moment. `a <$ f` is_
_the same as `const a <$> f`, but can be more efficient for some functors._
...
not go further to try `https`. The solution is to put the `SchemeHttps <$ string "https"` line before the `SchemeHttp <$ string "http"` line.
Remember: _with alternatives, order matters!_
...
## Controlling backtracking with `try` [Anchor](#controlling-backtracking-with-try)
...
Now we need to discuss an important concept called _backtracking_.
...
_I took the liberty of accepting any alpha-numeric sequences of characters_
_as username and password, and made similarly arbitrary simplifications in_
_the format of the host._
...
## Debugging parsers [Anchor](#debugging-parsers)
...
## Labeling and hiding things [Anchor](#labeling-and-hiding-things)
...
custom, typically called a _label_. This is done with the help of the
...
_EXERCISE: Finishing the `pUri` parser is left as an exercise for the_
_reader, now that all the tools that are necessary for this have been_
_explained._

## Running a parser [Anchor](#running-a-parser)
...
## The `MonadParsec` type class [Anchor](#the-monadparsec-type-class)

All tools in `megaparsec` work with any instance of the `MonadParsec` type
class. The type class abstracts _primitive combinators_—the elementary
...
get _backtracking state_ in the sense that if the first branch is discarded
...
reader context `r`. There is a difference though, because the expressions `m s` and `n s` produce stateful results: together with monadic value, they
...
provide that if we allow to wrap `ParsecT` _inside_`StateT`:
...
## Lexing [Anchor](#lexing)

_Lexing_ is the process of transforming the input stream into a stream of
...
### White space [Anchor](#white-space)
...
_space consumer_. The `Text.Megaparsec.Char.Lexer` module provides a helper
...
### Char and string literals [Anchor](#char-and-string-literals)
...
- To turn `L.charLiteral` into a parser for char literals we only need to
  ...
  quotes. The `between` combinator is defined simply as: `between open close p = open *> p <* close`.
...
## `notFollowedBy` and `lookAhead` [Anchor](#notfollowedby-and-lookahead)
...
If the argument `p` of `lookAhead` succeeds, the whole construct `lookAhead p` also succeeds but the input stream (and the entire parser state) stays
...
## Parsing expressions [Anchor](#parsing-expressions)
...
that, the [`parser-combinators`](https://hackage.haskell.org/package/parser-combinators) package comes with the
...
## Indentation-sensitive parsing [Anchor](#indentationsensitive-parsing)
...
### `nonIndented` and `indentBlock` [Anchor](#nonindented-and-indentblock)
...
It allows us to make sure that its inner parser consumes input that is _not_
...
here is that this space-consuming parser _must_ consume newlines as well,
...
We can change our mind and parse no indented tokens, we can parse _many_
(that is, possibly zero) indented tokens or require _at least one_ such
...
### Parsing a simple indented list [Anchor](#parsing-a-simple-indented-list)
...
Let’s replace `IndentMany` with `IndentSome` and `Nothing` with `Just (mkPos 5)` (indentation levels are counted from 1, so it will require 4 spaces
...
### Nested indented list [Anchor](#nested-indented-list)
...
### Adding line folds [Anchor](#adding-line-folds)

A _line fold_ consists of several elements that can be put on one line or on
...
_EXERCISE: Playing with the final version of our parser is left as an_
_exercise for the reader. You can create “items” that consist of multiple_
_words and as long as they are line-folded they will be parsed and_
_concatenated with single space between them._

## Writing efficient parsers [Anchor](#writing-efficient-parsers)
...
- Use the fast primitives such as `takeWhileP`, `takeWhile1P`, and `takeP`
  whenever you can. [This blog post](https://markkarpov.com/post/megaparsec-more-speed-more-power#there-is-hope) explains
...
## Parse errors [Anchor](#parse-errors)
...
### Parse error definitions [Anchor](#parse-error-definitions)
...
### How to signal a parse error [Anchor](#how-to-signal-a-parse-error)
...
### Displaying parse errors [Anchor](#displaying-parse-errors)
...
### Catching parse errors in a running parser [Anchor](#catching-parse-errors-in-a-running-parser)
...
exceptions](https://markkarpov.com/tutorial/exceptions). This is enabled by the `observing` primitive:
...
_EXERCISE: Understand in details how this program works._
...
indeed define _regions_ in which parse errors are processed in some way. The
...
_EXERCISE: Rewrite the `inside` function in the program above using_
_`region`._

### Controlling location of parse errors [Anchor](#controlling-location-of-parse-errors)
...
### Reporting multiple parse errors [Anchor](#reporting-multiple-parse-errors)
...
Megaparsec 8 supports _delayed parse errors_:
...
## Testing Megaparsec parsers [Anchor](#testing-megaparsec-parsers)
...
by using the [`hspec-megaparsec`](https://hackage.haskell.org/package/hspec-megaparsec) package. The package
...
_EXERCISE: Familiarize yourself with `errFancy`, which is used to construct_
_fancy parse errors._
...
- [Megaparsec’s own test suite](https://github.com/mrkkrp/megaparsec/tree/master/megaparsec-tests/tests) is written using
  `hspec-megaparsec`.

- The [toy test suite](https://github.com/mrkkrp/hspec-megaparsec/blob/master/tests/Main.hs) that comes with
  `hspec-megaparsec` itself.

## Working with custom input streams [Anchor](#working-with-custom-input-streams)
...
like this) can be found in [this blog post](https://markkarpov.com/post/megaparsec-more-speed-more-power). Note
